...
module SouthParkQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SouthParkQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = SouthParkQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SouthParkQuotesResponse.new(fetched)
    else
