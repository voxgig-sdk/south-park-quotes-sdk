...
// SouthParkQuotes SDK retry feature
...
class SouthParkQuotesRetryFeature extends SouthParkQuotesBaseFeature
{
    ...
    public function init(SouthParkQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SouthParkQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SouthParkQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SouthParkQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
