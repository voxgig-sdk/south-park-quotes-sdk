...
// SouthParkQuotes SDK feature test
//
...
class FtCtrl extends SouthParkQuotesControl
{
...
class FtClient
{
    ...
    private SouthParkQuotesUtility $utility;

    public function __construct(SouthParkQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): SouthParkQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SouthParkQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SouthParkQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SouthParkQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SouthParkQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(SouthParkQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SouthParkQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SouthParkQuotesResponse($fetched);
        }
    ...
    private function fail(SouthParkQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SouthParkQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
