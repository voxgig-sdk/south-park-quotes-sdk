...
//     (\`new SouthParkQuotesSDK(...)\` / \`SouthParkQuotesSDK.test(...)\` become
//     \`SouthParkQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SouthParkQuotesSDK'
...
// rewrite real client construction (\`new SouthParkQuotesSDK(...)\` and any
// \`SouthParkQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SouthParkQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SouthParkQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
