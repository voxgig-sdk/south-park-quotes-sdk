...
import (
	...
	omni "github.com/voxgig-sdk/south-park-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SouthParkQuotesError); is {
				out := map[string]any{
					"name":    "SouthParkQuotesError",
...
func omniCtx(arg any, client *sdk.SouthParkQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SouthParkQuotesSDK); is {
			client = live
