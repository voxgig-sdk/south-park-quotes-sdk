...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SOUTHPARKQUOTES_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SOUTHPARKQUOTES_TEST_QUOTE_ENTID")
	...
	if env["SOUTHPARKQUOTES_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewSouthParkQuotesSDK(core.ToMapAny(mergedOpts))
	}
