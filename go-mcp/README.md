# south-park-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SouthParkQuotes SDK as
two agent tools — `south-park-quotes_list` and `south-park-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/south-park-quotes-mcp)
make build

# 2. Provide credentials via the environment
export SOUTH_PARK_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user south-park-quotes \
  -- /absolute/path/to/south-park-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./south-park-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// south-park-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// south-park-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/south-park-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user south-park-quotes \
     -- "$PWD"/dist/*/south-park-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `south-park-quotes_list` and `south-park-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using south-park-quotes"*
   and it calls `south-park-quotes_list` with `{"entity":"quote"}`.
...
```sh
export SOUTH_PARK_QUOTES_APIKEY=sk_live_xxx            # API key
export SOUTH_PARK_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `south-park-quotes_list` tool
...
### Call the `south-park-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `south-park-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `south-park-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SOUTH_PARK_QUOTES_APIKEY` | API key sent with every request. |
| `SOUTH_PARK_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./south-park-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"south-park-quotes_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
