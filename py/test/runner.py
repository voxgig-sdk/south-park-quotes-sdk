# SouthParkQuotes SDK test runner
...
class SouthParkQuotesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SouthParkQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SouthParkQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SouthParkQuotesTestRunner.getenv("SOUTH_PARK_QUOTES_TEST_LIVE")
        override = SouthParkQuotesTestRunner.getenv("SOUTH_PARK_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SouthParkQuotesTestRunner.getenv(key)
        ...
        explain = SouthParkQuotesTestRunner.getenv("SOUTH_PARK_QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SOUTH_PARK_QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SouthParkQuotesTestRunner._test_control is not None:
            return SouthParkQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SouthParkQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SouthParkQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SouthParkQuotesTestRunner.load_test_control()
...
def load_env_local():
    SouthParkQuotesTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return SouthParkQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SouthParkQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SouthParkQuotesTestRunner.load_test_control()


def live_client_options():
    return SouthParkQuotesTestRunner.live_client_options()


def live_delay_ms():
    return SouthParkQuotesTestRunner.live_delay_ms()
