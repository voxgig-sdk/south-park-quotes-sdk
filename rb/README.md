...
```ruby
require_relative "SouthParkQuotes_sdk"
client = SouthParkQuotesSDK.new(options)
```
...
raise a `SouthParkQuotesError` (a `StandardError` subclass), so wrap
...
```
rb/
├── SouthParkQuotes_sdk.rb       -- Main SDK module
...
The main module (`SouthParkQuotes_sdk`) exports the SDK class
