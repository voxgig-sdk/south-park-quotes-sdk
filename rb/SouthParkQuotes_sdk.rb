...
require_relative 'SouthParkQuotes_types'
...
class SouthParkQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SouthParkQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SouthParkQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SouthParkQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SouthParkQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SouthParkQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SouthParkQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SouthParkQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SouthParkQuotesError.new(
        "#{op}_allow",
        "SouthParkQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue SouthParkQuotesError => err
    ...
    if fetched.is_a?(Hash)
      status = SouthParkQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SouthParkQuotesError.new(
        "graphql_error", "SouthParkQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SouthParkQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
