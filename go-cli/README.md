# south-park-quotes-cli

boru-driven command-line client **and** interactive REPL for the SouthParkQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/south-park-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./south-park-quotes-cli --help

# 3. Provide credentials once, via the environment
export SOUTH_PARK_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./south-park-quotes-cli list quote
./south-park-quotes-cli load 1 quote            # {id:1} shorthand
./south-park-quotes-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
SOUTH_PARK_QUOTES_BASE=https://api.example.com ./south-park-quotes-cli list quote

# 6. No arguments -> interactive REPL
./south-park-quotes-cli
south-park-quotes> list quote
south-park-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/south-park-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/south-park-quotes-cli list quote
   ```
...
```sh
./south-park-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./south-park-quotes-cli load '{id:1}' quote     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `south-park-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./south-park-quotes-cli
south-park-quotes> list quote
south-park-quotes> /help
south-park-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/south-park-quotes-cli`. |
...
(`./south-park-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
